...
@Component({
  ...
  template: `
    <div class="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <!-- Date Range Picker -->
      <div
        class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm shadow-slate-200/50 dark:shadow-none p-4 flex flex-wrap gap-4 items-end"
      >
        <div class="space-y-1.5">
          <label
            for="date-from"
            class="me-1 text-xs text-slate-500 dark:text-slate-400 font-medium"
            >{{ 'REPORTS.DATE_FROM' | translate }}</label
          >
          <input
            id="date-from"
            type="date"
        ...
        <div class="space-y-1.5">
          <label
            for="date-to"
            class="me-1 text-xs text-slate-500 dark:text-slate-400 font-medium"
            >{{ 'REPORTS.DATE_TO' | translate }}</label
          >
          <input
            id="date-to"
            type="date"
        ...
        <div class="space-y-1.5">
          <label
            for="filter-project"
            class="me-1 text-xs text-slate-500 dark:text-slate-400 font-medium"
            >{{ 'REPORTS.FILTER_PROJECT' | translate }}</label
          >
          <select
            id="filter-project"
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        @for (stat of summaryStats(); track stat.labelKey) {
          <div
            class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm shadow-slate-200/50 dark:shadow-none p-4 text-center"
          >
            <div class="text-xs text-slate-500 dark:text-slate-400 mb-1">
              {{ stat.labelKey | translate }}
            </div>
            <div class="text-xl font-semibold text-slate-900 dark:text-slate-100 font-mono">
              {{ stat.value }}
            </div>
          </div>
        }
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <!-- Project Breakdown -->
        <div
          class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm shadow-slate-200/50 dark:shadow-none"
        >
          <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
            <h2 class="font-semibold text-slate-900 dark:text-slate-100">
              {{ 'REPORTS.BY_PROJECT' | translate }}
            </h2>
          </div>
          <ul class="p-4 space-y-3" role="list">
            @for (p of projectBreakdown(); track p.id) {
                ...
                <div class="flex items-center justify-between text-sm">
                  ...
                  <div class="flex items-center gap-3">
                    <span class="text-xs text-slate-400 dark:text-slate-500"
                      >{{ p.count }} {{ 'REPORTS.ENTRIES_COUNT' | translate }}</span
                    >
                    <span class="font-mono text-slate-700 dark:text-slate-200">{{
                      p.totalSeconds | duration
                    }}</span>
              ...
              <li class="text-center text-slate-400 dark:text-slate-500 text-sm py-4">
        ...
        >
          ...
          <ul class="p-4 space-y-3" role="list">
            @for (c of categoryBreakdown(); track c.value) {
              ...
              <li class="text-center text-slate-400 text-sm py-4">
                {{ 'COMMON.NO_DATA' | translate }}
              </li>
            }
          </ul>
...
export class ReportsComponent {
  ...
  readonly presets = [
    {
      labelKey: 'REPORTS.PRESET_THIS_MONTH',
      from: () => startOfMonth(new Date()),
      to: () => endOfMonth(new Date()),
    },
    {
      labelKey: 'REPORTS.PRESET_LAST_MONTH',
      from: () => startOfMonth(subMonths(new Date(), 1)),
      to: () => endOfMonth(subMonths(new Date(), 1)),
    },
    {
      labelKey: 'REPORTS.PRESET_LAST_3M',
  ...
  readonly summaryStats = computed(() => {
    ...
    return [
      ...
      {
        labelKey: 'REPORTS.STAT_AVG_PER_DAY',
        value: days
          ? `${Math.floor(total / days / 3600)}h ${Math.floor(((total / days) % 3600) / 60)}m`
          : '0h',
      },
    ];
  });
    ...
    return this.projectService
      ...
      .map((p) => {
        ...
        return {
          ...
          percentage: Math.round((totalSeconds / total) * 100),
        };
      })
